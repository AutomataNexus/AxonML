...
fn main() {
    ...
    let archs: Vec<&str> = if arch == "all" {
        vec![
            "boreas", "aquilo", "naiad", "vulcan", "zephyrus", "colossus", "gaia", "apollo",
            "mnemosyne", "argus", "ariadne",
            "sentinel", "nabu", "birdclef", "atlas",
            "qwen3-4l", "rdt-tiny",
            "gpt2-tiny", "bert-tiny", "phi-tiny", "mistral-tiny",
            "mamba-ssm",
            "trident", "trident-1b", "trident-tcn",
            "hydra", "hydra-300m",
            "chimera", "chimera-2b",
            "olympus-position", "olympus-pvalue", "olympus-meta",
            "olympus-gene", "olympus-npz", "olympus-combined",
        ]
    } else {
        vec![arch.as_str()]
    ...
    for name in &archs {
        ...
        let params: usize = bundle.graph.as_ref().map(|g| g.initializers.values().map(|t| t.data.len()).sum()).unwrap_or(0);
...
fn build_model(name: &str) -> ModelBundle {
    match name {
        ...
        "boreas"   => build_hvac_diagnostic("boreas", "Refrigeration Diagnostics", 7, 16, 1, 5, 8, 80),
        "aquilo"   => build_hvac_diagnostic("aquilo", "Cooling Tower Diagnostics", 6, 12, 1, 4, 6, 60),
        "naiad"    => build_hvac_diagnostic("naiad", "Water Systems Diagnostics", 8, 14, 1, 6, 8, 80),
        "vulcan"   => build_hvac_diagnostic("vulcan", "Boiler/Furnace Diagnostics", 9, 18, 1, 7, 10, 80),
        "zephyrus" => build_hvac_diagnostic("zephyrus", "Air Handler Diagnostics", 11, 20, 1, 8, 11, 120),
        "colossus" => build_hvac_diagnostic("colossus", "Chiller Plant Diagnostics", 12, 22, 1, 9, 12, 120),
        "gaia"     => build_hvac_diagnostic("gaia", "Geothermal Diagnostics", 10, 16, 1, 6, 8, 80),
        "apollo"   => build_hvac_diagnostic("apollo", "Unified HVAC Diagnostics", 15, 24, 1, 10, 14, 120),
        ...
        "birdclef"  => build_vision_cnn("birdclef", "BirdClef SedNet 234-species", 1, 128, 256, 234, 5),
        ...
        "trident"       => build_transformer("trident", "Trident 1.58-bit Ternary LM (BitNet b1.58)", 64, 128, 4),
        "trident-1b"    => build_transformer("trident-1b", "Trident 1B Ternary (d=512, 12L)", 512, 1024, 12),
        "hydra"         => build_hydra("hydra", "Hydra SSM+Windowed Attention Hybrid", 64, 128, 4),
        "hydra-300m"    => build_hydra("hydra-300m", "Hydra 300M (d=768, 12L, S6+LocalAttn)", 768, 1536, 12),
        "chimera"       => build_chimera("chimera", "Chimera MoE+DiffAttn (8 experts, top-2)", 64, 128, 4, 8),
        "chimera-2b"    => build_chimera("chimera-2b", "Chimera 2B (d=512, 16L, 8 experts)", 512, 1024, 16, 8),
        "trident-tcn"   => build_tcn_transformer("trident-tcn", "Trident TCN 1.58-bit Temporal", 32, 64, 4),

        // Alzheimer's OLYMPUS genomic experts
        "olympus-position" => build_mlp("olympus-position", "OLYMPUS Position Expert", 128, &[256, 128, 64], 2),
        "olympus-pvalue"   => build_mlp("olympus-pvalue", "OLYMPUS P-value Expert", 64, &[128, 64], 2),
        "olympus-meta"     => build_mlp("olympus-meta", "OLYMPUS Meta Expert", 96, &[192, 96, 48], 2),
        "olympus-gene"     => build_mlp("olympus-gene", "OLYMPUS Gene Expert", 256, &[512, 256, 128], 2),
        "olympus-npz"      => build_mlp("olympus-npz", "OLYMPUS NPZ Expert", 384, &[768, 384, 192], 2),
        "olympus-combined" => build_mlp("olympus-combined", "OLYMPUS Combined Expert", 512, &[1024, 512, 256], 2),
...
fn build_hvac_diagnostic(name: &str, desc: &str, n_features: i64, n_classes: i64,
                         kh: i64, n_horizons: i64, hidden_ch: i64, seq_len: i64) -> ModelBundle {
    ...
    add_conv_bn_relu(&mut g, "enc2", h, h*2, 1, 3, 2, "enc1_out_relu", "enc2_out", 2);
    add_conv_bn_relu(&mut g, "enc3", h*2, h*2, 1, 3, 2, "enc2_out_relu", "enc3_out", 3);
    ...
    g.add_node("flatten", "Flatten", serde_json::json!({"axis": 1}),
               vec!["pooled"], vec!["flat"]);
    add_gemm(&mut g, "head_class", h*2, n_classes, "flat", "class_logits", 10);
...
fn build_vision_cnn(name: &str, desc: &str, in_ch: i64, h: i64, w: i64,
                    n_classes: i64, n_blocks: usize) -> ModelBundle {
    let mut g = BundleGraph::new();
    ...
    for i in 0..n_blocks.min(channels.len()) {
        ...
        add_conv_bn_relu(&mut g, &lbl, prev_ch, ch, 3, 3, 2, &prev_out, &out, (i+1) as u64);
        prev_ch = ch;
        prev_out = format!("{lbl}_out_relu");
    }

    g.add_node("gap", "GlobalAvgPool", serde_json::Value::Null,
               vec![&prev_out], vec!["pooled"]);
    g.add_node("flatten", "Flatten", serde_json::json!({"axis": 1}),
               vec!["pooled"], vec!["flat"]);
    add_gemm(&mut g, "classifier", prev_ch, n_classes, "flat", "logits", 100);
...
fn build_mlp(name: &str, desc: &str, in_dim: i64, hidden: &[i64], out_dim: i64) -> ModelBundle {
    ...
    for (i, &h) in hidden.iter().enumerate() {
        ...
        g.add_node(&format!("{lbl}_act"), "Relu", serde_json::Value::Null,
                   vec![&out], vec![&relu]);
        prev_dim = h;
        prev_name = relu;
    }
    add_gemm(&mut g, "output_head", prev_dim, out_dim, &prev_name, "output", 99);
...
fn build_transformer(name: &str, desc: &str, d_model: i64, d_inner: i64, n_layers: usize) -> ModelBundle {
    let mut g = BundleGraph::new();
    ...
    for i in 0..n_layers {
        ...
        add_conv1x1_bn_relu(&mut g, &qkv, d_model, d_inner, &prev, &qkv_out, (i*10+1) as u64);

        let proj = format!("layer{i}_proj");
        let proj_out = format!("{proj}_out");
        let proj_relu = format!("{proj}_relu");
        add_conv1x1_bn_relu(&mut g, &proj, d_inner, d_model, &format!("{qkv}_out_relu"), &proj_out, (i*10+2) as u64);
        prev = format!("{proj}_out_relu");
    }

    // Identity to output
    g.add_node("out_id", "Identity", serde_json::Value::Null,
               vec![&prev], vec!["output"]);
...
fn build_tcn_transformer(name: &str, desc: &str, d_model: i64, d_inner: i64, n_layers: usize) -> ModelBundle {
    let mut g = BundleGraph::new();
    ...
    for i in 0..n_layers {
        ...
        add_conv_bn_relu(&mut g, &conv, d_model, d_model, 1, 3, 1, &prev, &out, (i+1) as u64);
...
fn build_hydra(name: &str, desc: &str, d_model: i64, d_inner: i64, n_layers: usize) -> ModelBundle {
    ...
    for i in 0..n_layers {
        ...
        add_conv_bn_relu(&mut g, &ssm, d_model, d_inner, 1, 3, 1, &prev, &ssm_out, (i*10+1) as u64);

        // Gate: Conv1x1 projection back to d_model
        let gate = format!("layer{i}_gate");
        let gate_out = format!("{gate}_out");
        add_conv1x1_bn_relu(&mut g, &gate, d_inner, d_model, &format!("{ssm}_out_relu"), &gate_out, (i*10+2) as u64);

        // Windowed attention: Conv1x1 QKV projection
        let attn = format!("layer{i}_attn");
        let attn_out = format!("{attn}_out");
        add_conv1x1_bn_relu(&mut g, &attn, d_model, d_inner, &format!("{gate}_out_relu"), &attn_out, (i*10+3) as u64);

        // Output projection
        let proj = format!("layer{i}_proj");
        let proj_out = format!("{proj}_out");
        add_conv1x1_bn_relu(&mut g, &proj, d_inner, d_model, &format!("{attn}_out_relu"), &proj_out, (i*10+4) as u64);
        prev = format!("{proj}_out_relu");
    }
    g.add_node("out_id", "Identity", serde_json::Value::Null, vec![&prev], vec!["output"]);
...
fn build_chimera(name: &str, desc: &str, d_model: i64, d_inner: i64, n_layers: usize, n_experts: usize) -> ModelBundle {
    let mut g = BundleGraph::new();
    g.add_input("input", vec![-1, d_model, 4, 1]);
    g.add_output("output", vec![-1, d_model, 4, 1]);
    ...
    for i in 0..n_layers {
        ...
        add_conv1x1_bn_relu(&mut g, &attn_a, d_model, d_inner, &prev, &attn_a_out, (i*20+1) as u64);
        add_conv1x1_bn_relu(&mut g, &attn_b, d_model, d_inner, &prev, &attn_b_out, (i*20+2) as u64);

        // Output projection from attention
        let attn_proj = format!("layer{i}_attn_proj");
        let attn_proj_out = format!("{attn_proj}_out");
        add_conv1x1_bn_relu(&mut g, &attn_proj, d_inner, d_model, &format!("{attn_a}_out_relu"), &attn_proj_out, (i*20+3) as u64);
        ...
        for e in 0..n_experts.min(4) {
            ...
            add_conv1x1_bn_relu(&mut g, &up, d_model, d_inner / 2, &format!("{attn_proj}_out_relu"), &up_out, (i*20+10+e) as u64);

            let down = format!("layer{i}_expert{e}_down");
            let down_out = format!("{down}_out");
            add_conv1x1_bn_relu(&mut g, &down, d_inner / 2, d_model, &format!("{up}_out_relu"), &down_out, (i*20+14+e) as u64);
        }
...
fn add_conv_bn_relu(g: &mut BundleGraph, name: &str, in_c: i64, out_c: i64,
                    kh: i64, kw: i64, stride: i64, input: &str, output: &str, seed: u64) {
    ...
    g.add_initializer(&cw, vec![out_c, in_c, kh, kw], init_kaiming(w_n, (in_c*kh*kw) as usize, seed));
