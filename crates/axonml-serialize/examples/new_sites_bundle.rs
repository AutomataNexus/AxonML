...
fn add_conv_bn_relu_full(
    ...
    g.add_initializer(&cw, vec![out_c, in_c_per_group, 1, kernel],
    ...
    if bn {
        ...
        g.add_node(&format!("{name}_bn"), "BatchNorm",
            ...
            vec![&last, &bn_w, &bn_b, &bn_m, &bn_v], vec![&bn_out]);
        last = bn_out;
    }
...
fn add_cbr(
    ...
    add_conv_bn_relu_full(g, name, in_c, out_c, kernel, 1, 1, 1, in_act, out_prefix, true, true, seed)
}
...
fn add_cb(
    ...
    add_conv_bn_relu_full(g, name, in_c, out_c, kernel, 1, dilation, 1, in_act, out_prefix, true, false, seed)
}
...
fn add_pw_cbr(
    ...
    add_conv_bn_relu_full(g, name, in_c, out_c, 1, 1, 1, 1, in_act, out_prefix, true, true, seed)
}
...
fn add_pw_cb(
    ...
    add_conv_bn_relu_full(g, name, in_c, out_c, 1, 1, 1, 1, in_act, out_prefix, true, false, seed)
}
...
fn add_dw_cbr(
    ...
    add_conv_bn_relu_full(g, name, channels, channels, kernel, 1, 1, channels, in_act, out_prefix, true, true, seed)
}
...
fn add_residual(
    ...
    g.add_node(name, "Add", serde_json::Value::Null,
        vec![a, b], vec![out]);
}
...
fn add_gap_flatten(
    ...
    g.add_node(&format!("{prefix}_gap"), "GlobalAvgPool", serde_json::Value::Null,
        vec![in_act], vec![&gap_out]);
    g.add_node(&format!("{prefix}_flatten"), "Flatten",
...
fn build_taylor_greenhouse(seed_base: u64) -> ModelBundle {
    ...
    let b1 = add_cbr(&mut g, "branch1", n_features, 32, 3, "sensor_seq", "b1", seed_base + 1);

    // Branch 2 (humidity sensors): Conv[14->24, k5]
    let b2 = add_conv_bn_relu_full(&mut g, "branch2", n_features, 24, 5, 1, 1, 1,
        "sensor_seq", "b2", true, true, seed_base + 2);

    // Branch 3 (environmental): Conv[14->32, k3]
    let b3 = add_cbr(&mut g, "branch3", n_features, 32, 3, "sensor_seq", "b3", seed_base + 3);

    // Concat branches (32+24+32=88 channels)
    g.add_node("concat", "Concat", serde_json::json!({"axis": 1}),
        vec![&b1, &b2, &b3], vec!["concat_out"]);

    // Fusion layers
    let fuse1 = add_cbr(&mut g, "fuse1", 88, 64, 3, "concat_out", "fuse1", seed_base + 4);
    let fuse2 = add_conv_bn_relu_full(&mut g, "fuse2", 64, 48, 5, 1, 1, 1,
        &fuse1, "fuse2", true, true, seed_base + 5);

    // GAP + Flatten
    add_gap_flatten(&mut g, "pool", &fuse2, "flat");

    // 3 Gemm heads (48 -> 10 each)
    add_gemm_head(&mut g, "head_temp", 48, 10, "flat", "head_temp", seed_base + 10);
    add_gemm_head(&mut g, "head_humid", 48, 10, "flat", "head_humid", seed_base + 11);
    add_gemm_head(&mut g, "head_env", 48, 10, "flat", "head_env", seed_base + 12);
    ...
    ModelBundle::new("taylor_greenhouse", n_features as usize, Vec::new())
        ...
        .with_hyperparam("note", format!("Multi-branch 3-path sensor fusion; total_params={total_params}"))
        .with_graph(g)
}
...
fn build_taylor_natorium(seed_base: u64) -> ModelBundle {
    ...
    let stem = add_cbr(&mut g, "stem", n_features, hidden, 3, "sensor_seq", "stem", seed_base + 1);
    ...
    for (i, &d) in dilations.iter().enumerate() {
        ...
        let c1 = add_conv_bn_relu_full(&mut g, &format!("{blk}_c1"), hidden, hidden, 3, 1, d, 1,
            &last, &format!("{blk}_c1"), true, true, seed);

        // Conv[64->64, k3] + BN (no relu yet — add after residual)
        let c2 = add_cb(&mut g, &format!("{blk}_c2"), hidden, hidden, 3, 1, &c1, &format!("{blk}_c2"), seed + 1);
        ...
        g.add_node(&format!("{blk}_relu"), "Relu", serde_json::Value::Null,
            vec![&add_out], vec![&relu_out]);
    ...
    add_gemm_head(&mut g, "head_a", hidden, 12, "flat", "head_a", seed_base + 100);
    add_gemm_head(&mut g, "head_b", hidden, 12, "flat", "head_b", seed_base + 101);
    add_gemm_head(&mut g, "head_c", hidden, 12, "flat", "head_c", seed_base + 102);
    ...
    ModelBundle::new("taylor_natorium", n_features as usize, Vec::new())
        ...
        .with_hyperparam("note", format!("Deep residual with dilated convs; total_params={total_params}"))
        .with_graph(g)
}
...
fn build_taylor_chiller(seed_base: u64) -> ModelBundle {
    ...
    let stem = add_cbr(&mut g, "stem", n_features, wide, 3, "sensor_seq", "stem", seed_base + 1);
    ...
    for i in 0..4 {
        ...
        let c1 = add_pw_cbr(&mut g, &format!("{blk}_sq"), wide, narrow, &last, &format!("{blk}_sq"), seed);

        // Conv[32->32, k5] + BN + ReLU
        let c2 = add_conv_bn_relu_full(&mut g, &format!("{blk}_mid"), narrow, narrow, 5, 1, 1, 1,
            &c1, &format!("{blk}_mid"), true, true, seed + 1);

        // Conv1x1[32->96] + BN (no relu — add after residual)
        let c3 = add_pw_cb(&mut g, &format!("{blk}_ex"), narrow, wide, &c2, &format!("{blk}_ex"), seed + 2);
    ...
    add_gemm_head(&mut g, "head_perf", wide, 8, "flat", "head_perf", seed_base + 100);
    add_gemm_head(&mut g, "head_fault", wide, 8, "flat", "head_fault", seed_base + 101);
    ...
    ModelBundle::new("taylor_chiller", n_features as usize, Vec::new())
        ...
        .with_hyperparam("note", format!("Bottleneck squeeze-expand residual; total_params={total_params}"))
        .with_graph(g)
}
...
fn build_peabody_cooling_towers(seed_base: u64) -> ModelBundle {
    ...
    for i in 0..3 {
        ...
        let dw_outs: Vec<String> = kernels.iter().enumerate().map(|(j, &k)| {
            add_dw_cbr(&mut g, &format!("{blk}_dw{j}"), hidden, k, &last,
                &format!("{blk}_dw{j}"), seed + j as u64)
        }).collect();

        // Concat(72*3=216)
        let concat_out = format!("{blk}_cat");
        let dw_refs: Vec<&str> = dw_outs.iter().map(|s| s.as_str()).collect();
        g.add_node(&format!("{blk}_concat"), "Concat",
            serde_json::json!({"axis": 1}),
            dw_refs, vec![&concat_out]);

        // Pointwise Conv[216->72] + BN + ReLU
        let pw = add_pw_cbr(&mut g, &format!("{blk}_pw"), hidden * 3, hidden, &concat_out,
            &format!("{blk}_pw"), seed + 5);
    ...
    let red = add_cbr(&mut g, "reduce", hidden, 48, 3, &last, "reduce", seed_base + 50);

    // GAP + Flatten
    add_gap_flatten(&mut g, "pool", &red, "flat");

    // 2 Gemm heads (48 -> 10 each)
    add_gemm_head(&mut g, "head_eff", 48, 10, "flat", "head_eff", seed_base + 100);
    add_gemm_head(&mut g, "head_health", 48, 10, "flat", "head_health", seed_base + 101);
    ...
    ModelBundle::new("peabody_cooling_towers", n_features as usize, Vec::new())
        ...
        .with_hyperparam("n_heads", 2)
        .with_hyperparam("n_outputs_per_head", 10)
        .with_hyperparam("note", format!("Multi-scale depthwise separable; total_params={total_params}"))
        .with_graph(g)
}
...
fn build_peabody_boilers(seed_base: u64) -> ModelBundle {
    ...
    let stem = add_conv_bn_relu_full(&mut g, "stem", n_features, wide, 5, 1, 1, 1,
        "sensor_seq", "stem", true, true, seed_base + 1);
    ...
    let dt1 = add_conv_bn_relu_full(&mut g, "dil1", wide, wide, 7, 1, 2, 1,
        &last, "dil1", true, true, seed_base + 50);

    // Conv[80->64, k5, d=2] + BN + ReLU
    let dt2 = add_conv_bn_relu_full(&mut g, "dil2", wide, 64, 5, 1, 2, 1,
        &dt1, "dil2", true, true, seed_base + 51);

    // GAP + Flatten
    add_gap_flatten(&mut g, "pool", &dt2, "flat");

    // 3 Gemm heads (64 -> 10 each)
    add_gemm_head(&mut g, "head_eff", 64, 10, "flat", "head_eff", seed_base + 100);
    add_gemm_head(&mut g, "head_safety", 64, 10, "flat", "head_safety", seed_base + 101);
    add_gemm_head(&mut g, "head_maint", 64, 10, "flat", "head_maint", seed_base + 102);
    ...
    ModelBundle::new("peabody_boilers", n_features as usize, Vec::new())
        ...
        .with_hyperparam("n_heads", 3)
        .with_hyperparam("n_outputs_per_head", 10)
        .with_hyperparam("note", format!("Progressive compression + dilated temporal; total_params={total_params}"))
        .with_graph(g)
}
...
fn main() {
    ...
    let models: Vec<(&str, ModelBundle)> = vec![
        ...
        ("peabody_cooling_towers",  build_peabody_cooling_towers(0xbeef_0004)),
    ...
    for (slug, bundle) in &models {
        ...
        println!("{slug:30}  params={n_params:>8}  nodes={n_nodes:>3}  initializers={n_init:>3}  file={size:>10} bytes");
    }
    println!("---");
