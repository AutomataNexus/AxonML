...
fn main() {
    ...
    for (i, (name, in_c, out_c, in_act, out_act)) in enc_convs.iter().enumerate() {
        ...
        graph.add_initializer(&cw, vec![*out_c, *in_c, 3, 3],
            init_kaiming(n, (*in_c * 9) as usize, i as u64));
        graph.add_initializer(&cb, vec![*out_c], vec![0.0; *out_c as usize]);

        graph.add_initializer(&format!("{bn}.weight"), vec![*out_c], vec![1.0; *out_c as usize]);
        graph.add_initializer(&format!("{bn}.bias"), vec![*out_c], vec![0.0; *out_c as usize]);
        graph.add_initializer(&format!("{bn}.running_mean"), vec![*out_c], vec![0.0; *out_c as usize]);
        graph.add_initializer(&format!("{bn}.running_var"), vec![*out_c], vec![1.0; *out_c as usize]);
        ...
        graph.add_node(&bn, "BatchNorm",
            serde_json::json!({"epsilon":1e-5,"momentum":0.1}),
            vec![out_act, &format!("{bn}.weight"), &format!("{bn}.bias"),
                 &format!("{bn}.running_mean"), &format!("{bn}.running_var")],
            vec![&bn_out]);
        graph.add_node(&relu, "Relu", serde_json::Value::Null,
    ...
    graph.add_node("enc_gap", "GlobalAvgPool", serde_json::Value::Null,
        vec!["enc_relu3_out"], vec!["enc_pooled"]);
    graph.add_node("enc_flatten", "Flatten", serde_json::json!({"axis":1}),
        vec!["enc_pooled"], vec!["enc_flat"]);

    graph.add_initializer("bottleneck.weight", vec![16, 64], init_kaiming(16 * 64, 64, 10));
    graph.add_initializer("bottleneck.bias", vec![16], vec![0.0; 16]);
    graph.add_node("bottleneck", "Gemm",
        serde_json::json!({"alpha":1.0,"beta":1.0,"trans_a":false,"trans_b":true}),
        vec!["enc_flat", "bottleneck.weight", "bottleneck.bias"],
        vec!["latent_raw"]);
    graph.add_node("bottleneck_relu", "Relu", serde_json::Value::Null,
        vec!["latent_raw"], vec!["latent"]);
    ...
    let bundle = ModelBundle::new("prometheus_sae", 1, Vec::new())
        ...
        .with_hyperparam("description", "Prometheus Sparse Autoencoder — Conv2d encoder, \
    ...
    eprintln!("  Total params: {total_params} ({:.1} KB)", total_params as f64 * 4.0 / 1024.0);
    eprintln!("  Bundle: {}", final_path.display());
    eprintln!("  Size: {} bytes", std::fs::metadata(&final_path).unwrap().len());
}
