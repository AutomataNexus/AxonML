...
**axonml-hvac** is a domain-specific sub-crate of the AxonML deep learning framework providing nine specialized neural network models for HVAC fault detection, predictive maintenance, and facility-wide diagnostic reasoning. It was factored out of the core `axonml` umbrella crate in April 2026 to keep the framework itself domain-agnostic.
...
| `panoptes_datagen` | Warren HVAC simulator — physics-informed training data |
| `pipeline` | End-to-end 8-model diagnostic pipeline orchestration |
...
```rust
use axonml_hvac::{Apollo, Aquilo, Boreas, Panoptes, HvacPipeline};
...
GPU acceleration via the `cuda` feature flag:
