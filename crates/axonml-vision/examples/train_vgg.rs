...
use axonml_vision::models::vgg::vgg11_config;
use axonml_vision::models::{VggClassifier, VggFeatures};
...
const NUM_TEST: usize = 128;
const CHECKPOINT_DIR: &str = "checkpoints/vgg11_cifar10";
...
///
/// With 5 max-pool layers (stride 2), 32x32 spatial dims become 1x1,
/// so the classifier input is 512 * 1 * 1 = 512 features.
struct CifarVGG11 {
...
impl CifarVGG11 {
    fn new(num_classes: usize) -> Self {
        Self {
            features: VggFeatures::new(&vgg11_config(), true),
            classifier: VggClassifier::with_input_size(512, num_classes),
        }
    }
}
...
impl Module for CifarVGG11 {
    fn forward(&self, x: &Variable) -> Variable {
        ...
        let out = out.reshape(&[batch_size, features]);
...
fn main() {
    ...
    let monitor = if use_monitor {
        Some(
            TrainingMonitor::new("VGG-11 CIFAR-10", total_params)
