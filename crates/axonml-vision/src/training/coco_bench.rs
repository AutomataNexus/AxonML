...
//!
//! Full detection-model evaluation harness. Converts `Detection` boxes to
//! normalized `DetectionResult`s and `CocoAnnotation`s to `GroundTruth`, then
//! computes `(mAP@50, COCO mAP@[0.5:0.95])` via `compute_map` /
//! `compute_coco_map`. The internal `FullEvalResult` struct carries per-model
//! statistics (param count, image / detection counts, mean / P50 / P95
//! latency, FPS, mAP@50, mAP@75, COCO mAP, average box area and confidence).
//! Helpers `diverse_gt` synthesizes deterministic pseudo-random ground truth
//! with mixed small / medium / large boxes, `make_input` builds varied
//! gradient-pattern input tensors, `run_full_eval` drives warmup + timed
//! iteration + statistics aggregation, and `print_summary_table` /
//! `print_detection_stats` emit human-readable summary tables. Tests cover a
//! NanoDet multi-resolution sweep and a synthetic mAP-pipeline validation.
//!
//! # File
