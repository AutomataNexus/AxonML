    ...
    fn from_args() -> Self {
        ...
        while i < args.len() {
            match args[i].as_str() {
                ...
                "--partial-rotary" => {
                    i += 1;
                    cfg.partial_rotary = args[i].parse().unwrap();
                }
                "--no-bias" => {
                    cfg.use_bias = false;
                }
...
fn main() {
    ...
    if cfg.partial_rotary < 1.0 {
        eprintln!();
        eprintln!(
            "⚠  --partial-rotary {} < 1.0 will hit a framework bug at",
            cfg.partial_rotary
        );
    ...
    println!(
        "  head_dim        : {} (rotary_dim {} = {:.0}% partial RoPE)",
        head_dim,
        rotary_dim,
        cfg.partial_rotary * 100.0
    );
