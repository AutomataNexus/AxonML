...
- Tensor CPU inference optimization: hot activation/elementwise fallbacks (relu, sigmoid, tanh, exp, ln, neg and similar) now take direct storage slices for contiguous+offset0 tensors (no to_vec copy + alloc before feeding the already-parallel CpuBackend). zip_map / zip_map3 / map (primary for many GradFn bwd and custom elementwise) also use direct slices for both inputs when contiguous. Same fast-path pattern as matmul 2D. Big win for pure-CPU repeated forward passes (inference) and CPU GradFn bwd (fewer copies before rayon CpuBackend work).
- Measurement (improved harness): direct /proc polling sampler (bypassing external script file issues) + AXONML_PROFILE_BACKWARD=1 on mixed workload (autograd activation/softmax tests hitting new parallel bwds + 4-epoch CPU training example + full llm lib tests). 418 samples over ~16s window, peak 52 threads (rayon scaling), cpu_user accumulation ~63s (clear multi-core on parallel SwiGLU/softmax/narrow/matmul bwd paths during LLM-like training step proxy). CSV at /tmp/cpu_llm_step_sig_*.csv with ts/threads/rss/cpu. Confirms single-node CPU training/inference now using many cores on the hot GradFn + compute paths.
- Re-run measurement with full accumulated wins (incl. SumDim + tensor zip_map fastpaths): 436 samples, peak 40 threads, continued high cpu accumulation on the parallel paths. Multiple sig CSVs available for correlation with AXONML_PROFILE_BACKWARD output.
- Full llm lib test (127) green (~50s real Qwen work exercising all the CPU parallel GradFn (swiglu/softmax/narrow/sumdim) + tensor fastpaths + matmul threading). Part of every rolling cycle.
- All crates green post-changes (112 tensor, 132 autograd, 127 llm ~51-53s real Qwen work).
