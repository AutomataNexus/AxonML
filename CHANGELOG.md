...
- Measurement: sampler harness + `AXONML_PROFILE_BACKWARD=1` + direct autograd softmax/activation tests (hit the new parallel bwd paths) + short pure-CPU training example (4 epochs, real SwiGLU bwd + linears + bwd). Full `axonml-llm` lib runs (50s+, Qwen3 forwards exercising SwiGLU/softmax/attention/matmul on CPU) + prior matmul work. All crates green post-changes (112 tensor, 132 autograd, 127 llm).
