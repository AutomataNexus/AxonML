...
- Tensor CPU inference optimization: hot activation/elementwise fallbacks (relu, sigmoid, tanh, exp, ln, neg and similar) now take direct storage slices for contiguous+offset0 tensors (no to_vec copy + alloc before feeding the already-parallel CpuBackend). Uses the same fast-path pattern as matmul 2D. Big win for pure-CPU repeated forward passes (inference prefill/decode on large activations/weights) — copies eliminated before the rayon work.
