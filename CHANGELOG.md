...
- Measurement (improved harness): direct /proc polling sampler + AXONML_PROFILE_BACKWARD=1 on mixed workload (autograd + training example + full llm lib tests). Fresh run with reduce_grad_for_broadcast parallel: 419 samples, peak 52 threads, 40.2s cpu_user (scaling on the bwd paths). Multiple /tmp/cpu_llm_step_sig_*.csv available.
