...
- Measurement (improved harness): direct /proc polling sampler + AXONML_PROFILE_BACKWARD=1 on mixed workload (autograd + training example + full llm lib tests). Fresh run with rnn gradfn parallel: 426 samples, peak 52 threads, 63.8s cpu_user (scaling on the paths). Multiple /tmp/cpu_llm_step_sig_*.csv available.
