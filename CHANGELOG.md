...
- perf(cpu): VarDimBackward (variance/mean-dim GradFn, core for RMS/LayerNorm in LLM training) CPU grad pass now parallel with rayon. Single-node (CPU) LLM training step proxy measurement (attention + var_dim backward) with AXONML_PROFILE_BACKWARD + time; harness + sampler for full util sig. All relevant tests green. Continues GradFn/CPU parallel FAF (distributed bottom tier per usage).
